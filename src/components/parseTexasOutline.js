...
// Function to get color based on susceptible count
...
const InitialMap = () => {
  ...
  const onEachCounty = (feature, layer) => {
    ...
    if (countyInfo) {
      layer.bindTooltip(`${name}: ${countyInfo.susceptible} susceptible`, { permanent: false, direction: 'auto' });
  ...
  const geoJsonStyle = (feature) => ({
    ...
    weight: 1,
    color: 'white',
    dashArray: '4', // Ensure this is set for dashed lines
    fillOpacity: 0.7
  });
